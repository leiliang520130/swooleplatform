...
class Session
{
     ...
     * @return NULL
     */
    public function __construct($cache_life = 0, $cache = null)
    {
        $this->cache = $cache;
    ...
    public function start($readonly = false)
    {
        ...
        if(empty($sessid))
        {
            ...
            if($this->cache_life)
                Cookie::set(self::$cookie_key, $sessid, $this->cache_life, time());
            else
                Cookie::set(self::$cookie_key, $sessid, $this->cache_life);
        }
    ...
    {
        global $glargv;
        $session = $glargv['rediscon']->get(self::$cache_prefix.$sessId);
    ...
    public function set($sessId, $session='')
    {
        ...
        global $glargv;
        $ret = $glargv['rediscon']->set($key, $session, array('ex'=>86400));
        return $ret;
    }
    /**
     * 销毁Session
    ...
    public function delete($sessId = '')
    {
        global $glargv;
        unset($_SESSION);
        return $glargv['rediscon']->del(self::$cache_prefix.$this->sessID);
    }
    /**
     * 内存回收
     * @param   NULL
