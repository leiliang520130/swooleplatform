...
function ws_decode($pack, $packtype=1)
{
    if($packtype == 1)
        ...
        return msgpack_unpack($pack); //是msgpack_serialize的别名,序列号
}
...
function s2s_decode($pack)
{
    global $glconf;
    $packtype = 1;
    if(isset($glconf['s2spacktype'])) //服务器和服务器之间包协议序列化方法 值为1
        $packtype = (int)$glconf['s2spacktype'];
    if($packtype == 1)
        $req = json_decode($pack, true); //当该参数为 TRUE 时，将返回 array 而非 object
    else
        $req = msgpack_unpack($pack); //序列号
    if(is_array($req) and isset($req['cmd']) and is_string($req['cmd']))
        return $req;
    else
    {
        if(DEBUG) //是否开启调试,默认为0
        {
            debugvar($pack);
            debugvar($req);
        }
...
{
    global $glconf;
    if(isset($glconf['gamename'])) //platform
        $name = str_replace('php ', 'php ' . $glconf['gamename'] . '::', $name);
    if(PHP_OS!='Darwin')//MacOs除外
...
function debugvar($var, $str=false)
{
    global $glargv;
    //是否填充 "object" 的索引
    $a = debug_backtrace(DEBUG_BACKTRACE_PROVIDE_OBJECT, 1);//debug_backtrace() 产生一条 PHP 的回溯跟踪(backtrace)。
